...
# Define the minimal GRU (minGRU)
class minGRUCell(nn.Module):
    def __init__(self, input_size, hidden_size):
        super(minGRUCell, self).__init__()
        self.hidden_size = hidden_size
        self.z_linear = nn.Linear(input_size, hidden_size)
        self.h_tilde_linear = nn.Linear(input_size, hidden_size)

    def forward(self, x, h_prev):
        z_t = torch.sigmoid(self.z_linear(x))
        h_tilde = self.h_tilde_linear(x)
...
class ARCModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_size=NUM_COLORS):
        ...
        self.min_gru_cell = minGRUCell(input_size, hidden_size)
    ...
    def forward(self, x):
        ...
        h = torch.zeros(x.size(0), x.size(2), device=DEVICE)  # Initial hidden state
        # Process sequence in parallel using scan
        h_list = []
        for t in range(x.size(1)):
            h = self.min_gru_cell(x[:, t, :], h)
...
# Save model
torch.save(model.state_dict(), 'arc_min_gru_model.pth')
